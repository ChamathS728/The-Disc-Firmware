...
void requestDiscStatus(void) {
	...
	memcpy(packetPtr, txStrelkaSPI, sizeof(*packetPtr));

	// Run a regular Transmit Receive - receive isn't important - timeout of 100ms
	HAL_GPIO_WritePin(SPI1_CS_GPIO_Port, SPI1_CS_Pin, GPIO_PIN_RESET);
	HAL_SPI_TransmitReceive(SPICommsHandle, txStrelkaSPI, rxStrelkaSPI, sizeof(txStrelkaSPI), 100);
...
void transmitTargetPosition(PacketMove_t* packetPtr, uint32_t timestamp, uint16_t position) {
	/*
	...
	memcpy(&packet, txStrelkaSPI, sizeof(packet));
